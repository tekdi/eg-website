...
(function (window, videojs) {
  ...
  var scrollerProto = (function (plugin) {
    ...
    var easeOut = function (time, start, change, duration) {
      return (
        start + change * Math.sin(Math.min(1, time / duration) * (Math.PI / 2))
      );
    };
    ...
    var scrollToElement = function (element) {
      if (this.canScroll()) {
        ...
        var relBottom =
          element.offsetTop + element.clientHeight - parent.offsetTop;
        var newPos;
  ...
  var widget = (function (plugin) {
      ...
      selector.addEventListener("change", function (e) {
        setTrack(
          document.querySelector(
            "#" + plugin.prefix + "-" + plugin.player.id() + " option:checked"
          ).value
        );
    ...
    var clickToSeekHandler = function (event) {
      ...
      var clickedTime =
        event.target.getAttribute("data-begin") ||
        event.target.parentElement.getAttribute("data-begin");
      if (clickedTime !== undefined && clickedTime !== null) {
        // can be zero
        if (
          plugin.settings.clickArea === "line" || // clickArea: 'line' activates on all elements
          (plugin.settings.clickArea === "timestamp" &&
            clickedClasses.contains(plugin.prefix + "-timestamp")) ||
          (plugin.settings.clickArea === "text" &&
            clickedClasses.contains(plugin.prefix + "-text"))
        ) {
      ...
      for (i = 0; i < lines.length; i++) {
        ...
        if (time > begin && time < end) {
          if (!line.classList.contains("is-active")) {
            ...
            if (
              plugin.settings.autoscroll &&
              !(plugin.settings.stopScrollWhenInUse && my.body.scroll.inUse())
            ) {
