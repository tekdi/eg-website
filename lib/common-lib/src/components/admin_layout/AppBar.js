...
const coharts = [
  { id: 1, name: 'Cohort 2022-23' },
  { id: 2, name: 'Cohort 2023-24' }
]
...
export default function AppBar({
              ...
              coharts.find(
                (e, i) =>
                  (localStorage.getItem('academic_year_id') === '' &&
                    i === 0) ||
                  e.id === localStorage.getItem('academic_year_id')
              )?.name
            }
          </AdminTypo.H5>
          <IconByName
  ...
  const handleGetCohort = (cohort) => {
    localStorage.setItem('academic_year_id', cohort?.id)
    getCohort(cohort)
  }
      ...
      >
        ...
        <HStack alignItems={'flex-end'} space='5'>
          ...
          <Stack px='2' justifyContent='flex-end' display='contents'>
            ...
            >
              {coharts.map((item) => (
                <Menu.Item key={item?.id} onPress={() => handleGetCohort(item)}>
                  {item?.name}
                </Menu.Item>
              ))}
            </Menu>
