...
export const generateUUID = () => {
  ...
  return 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, function (c) {
    ...
    let r = array[0] % 16 // Generates a random number between 0 and 15
...
export const getUniqueArray = (item) => {
  ...
  if (Array.isArray(arr)) {
    return arr?.filter((val, index) => {
      return arr.indexOf(val) === index && val !== 'null'
    })
  } else {
    return []
  }
...
export const setLocalUser = ({
  ...
  const storedFullName = fullName || `${first_name} ${last_name}`
...
export const logout = () => {
  ...
  localStorage.removeItem('benificiary')
  localStorage.removeItem('leaner_filter_prerak')
  // localStorage.clear()
}
...
export const validation = ({ data, key, errors, message, type = '' }) => {
  ...
  switch (type) {
    ...
    case 'required':
      boolean = !!data // Simplified check
      break

    default:
      {
        const regex = /^(?!.*[\u0900-\u097F])[A-Za-z0-9\s\p{P}]+$/
        boolean = !regex.exec(data)
      }
      break
  }
...
export const getBeneficaryDocumentationStatus = (docStatus) => {
  if (!docStatus) {
    ...
    return isAllCompleteOrNotApplicable
  }
}
...
export const getEnrollmentIds = (dataArr, state_name) => {
  if (Array?.isArray(dataArr)) {
    if (state_name === 'RAJASTHAN') {
      return {
        payment_receipt_document_id: dataArr?.find((e) =>
          [
