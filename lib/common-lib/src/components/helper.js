...
export const checkAadhaar = (user, aadhaar, compareList = []) => {
  ...
  const data = arr.map((item) => {
    ...
    if (item === 'first_name') {
      ...
      result = {
        name: 'DATE_OF_BIRTH',
        aadhaar:aadhaar?.dateOfBirth? moment(aadhaar?.dateOfBirth, 'D-M-Y').format('Y-M-D'):"-",
        user: user?.[item] || '-',
        arr: item
      }
      ...
      result = {
        name: 'GENDER',
        aadhaar: aadhaar?.gender?aadhaar?.gender:"-",
        user: user?.[item] || '-',
        arr: item
      }
      if (compareList?.includes('dob')) {
      ...
      result = {
        name: 'AADHAAR_NO',
        aadhaar: aadhaar?.maskedNumber?aadhaar?.maskedNumber:"-",
        user: user?.aadhar_no,
        arr: item
      }
