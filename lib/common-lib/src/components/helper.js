...
export const setSelectedAcademicYear = (data) => {
  try {
    if (data) {
      const stringData = typeof data === 'string' ? data : JSON.stringify(data)
      localStorage.setItem('academic_year_id', stringData)
      return true
    } else {
      return false
    }
...
export const getSelectedAcademicYear = () => {
  try {
    return jsonParse(localStorage.getItem('academic_year_id'))
  } catch (error) {
    return {}
  }
}

export const setSelectedProgramId = (data) => {
  try {
    if (data) {
      const stringData = JSON.stringify(data)

      localStorage.setItem('program_id', stringData)
      return true
    } else {
      return false
    }
...
export const getSelectedProgramId = () => {
  try {
    return jsonParse(localStorage.getItem('program_id'))
  } catch (error) {
    return {}
  }
}
