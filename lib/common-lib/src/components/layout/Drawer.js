...
import { FrontEndTypo, telemetryFactory } from '../..'
  ...
  const interactLogout = () => {
    const telemetryInteraction = {
      ...
      edata: {
        type: 'CLICK',
        id: 'logout-button',
        userName: [facilitator?.first_name, facilitator?.last_name].join(' '),
...
const SubMenu = memo(({ menus, ...props }) => {
  ...
  return (
    <VStack space={4}>
      ...
      {open && menus?.map((item) => <EGMenuItem key={item} {...item} />)}
    </VStack>
  )
})
