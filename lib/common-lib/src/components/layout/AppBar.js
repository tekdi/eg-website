...
export default function AppBar({
  ...
  const [prerakID, setPrerakId] = React.useState()

  useEffect(() => {
    try {
      const prerakId = localStorage.getItem('id')
      setPrerakId(prerakId)
    } catch (error) {
      console.error('Failed to fetch prerakID from localStorage:', error)
    }
  }, [])
  const toggleDrawer = () => {
  ...
  return (
    <Box borderBottomWidth='1' borderBottomColor={'gray.300'}>
      <Banner />
      <HStack justifyContent='space-between' bg='textMaroonColor.600'>
        ...
        >
          ...
          <HStack alignItems={'center'}>
            {!searchInput && isEnableSearchBtn ? (
              <Pressable onPress={(e) => handleSeachState(true)}>
                <IconByName
                  name='SearchLineIcon'
                  color='textMaroonColor.600'
                  size='sm'
