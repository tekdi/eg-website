export default function interfaceData(
  ...
  const data = {}
  for (let [key1, value1] of Object.entries(formate)) {
    if (!reverse) {
      if (key1 === 'mergeParameterWithDefaultValue') {
        for (let [mKey1, mValue1] of Object.entries(value1)) {
          data[mKey1] = mValue1
        }
      } else if (key1 === 'mergeParameterWithValue') {
        for (let [mvKey1, mvValue1] of Object.entries(value1)) {
        ...
        arr.forEach((item) => {
          if (value[item]) {
            ...
            if (formate['defaultValueSet'][item]) {
              if (formate['defaultValueSet'][item] === 'undefined') {
                value = undefined
              } else if (formate['defaultValueSet'][item] === 'null') {
                value = null
              } else {
