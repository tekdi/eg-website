...
export const marital_status = async (params = {}, header = {}) => {
  let headers = {
    Authorization: 'Bearer ' + localStorage.getItem('token'),
    ...header
  }
  const result = await get(
    `${process.env.REACT_APP_API_URL}/enum/enum_value_list?key=BENEFICIARY_MARITAL_STATUS`,
    { params, headers }
  )
  if (result.data) {
    return result.data.data
...
export const social_status = async (params = {}, header = {}) => {
  let headers = {
    Authorization: 'Bearer ' + localStorage.getItem('token'),
    ...header
  }
  const result = await get(
    `${process.env.REACT_APP_API_URL}/enum/enum_value_list?key=BENEFICIARY_SOCIAL_STATUS`,
    { params, headers }
  )
  if (result.data) {
    return result.data.data
