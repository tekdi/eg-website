  ...
  try {
    ...
    let newData = {
      ...
      core_faciltators: {
        ...
        has_diploma: data?.has_diploma,
        diploma_details: data?.diploma_details ? data?.diploma_details : null
      },
      extended_users: {
      ...
      qualification: {
        qualification_master_id: data?.qualification_master_id,
        qualification_reference_document_id:
          data?.qualification_reference_document_id
      },
      lang: data?.lang
    }
    const result = await post(
...
export const getPrerakCertificateDetails = async (params = {}, header = {}) => {
  try {
    let headers = {
      ...header,
      Authorization: 'Bearer ' + localStorage.getItem('token')
    ...
    const result = await get(
      `${process.env.REACT_APP_API_URL}/events/${params?.id}/get-events-by-user_id`,
      { params, headers }
    )
    if (result.data) {
      return result.data
...
export const checkPrerakStatusChange = async (
  { ...params } = {},
  header = {}
) => {
  try {
    ...
    const result = await post(
      `${process.env.REACT_APP_API_URL}/facilitators/admin/status-change-validation`,
      params,
      {
        headers: headers
      }
